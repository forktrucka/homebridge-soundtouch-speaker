...
| 2026-07-19 | 🔴 Real-device QA found a severe regression this fix newly exposed: `_scheduleNext` passed `_msUntilNextCron()`'s raw ms straight to `setTimeout` with no upper-bound clamp, and Node silently clamps any delay over `2147483647` (~24.8 days) to ~1ms — since the callback re-schedules itself immediately, a day-of-month/month schedule whose next run is >~25 days out became an infinite tight retry loop. Confirmed on real hardware (7,352+ sync attempts against a live speaker in under 3 minutes). Full record: `.claude/qa/2026-07-18-pre-release-test-plan.md`. Blocked `dev → beta` promotion. | Reproduced against real hardware during pre-beta QA; `TimeoutOverflowWarning` in logs confirmed the mechanism | — |
| 2026-07-19 | Fixed in `fix/preset-sync-timeout-overflow`: `_scheduleNext` now captures the target `Date.now() + ms` once, and a new `_scheduleChunk` helper chains `setTimeout` hops of at most `MAX_TIMEOUT_MS` (2,147,483,647) toward that target, only calling `sync()`/re-scheduling the next cron occurrence on the final hop that actually reaches it. `_msUntilNextCron`'s public signature and the existing 24h-fallback behavior are unchanged. Added a regression test (`#start and #stop`) that mocks `_msUntilNextCron` to return a 30-day delay, asserts `sync` does not fire again after a 1ms advance (would catch the old clamp-to-~1ms bug), and does fire exactly once after advancing through the full chain. Full gate green: `typecheck`, `lint`, `test` (470 passed) | — | Recomputing `_msUntilNextCron` from scratch on every intermediate hop (rejected — would drift the target time and re-run cron resolution unnecessarily on every ~24.8-day hop) |
...
- [x] Fix `_scheduleNext` setTimeout overflow (chunk delays above the 32-bit
      max instead of clamping into a tight retry loop); add regression test
