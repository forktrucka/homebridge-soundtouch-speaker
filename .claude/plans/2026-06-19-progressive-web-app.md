...
| 2026-07-17 | **Spike A real-device capture (Wireless Link Adapter): documented 8–10s setup-mode hold is wrong for this unit — actual threshold is ~2–4s** | Live testing: a 10–12s hold landed on the "disable networking" state (LED flashes once then turns fully off) instead of setup mode, matching the manual's own warning about the two operations sharing the same nominal duration. A retry at ~2–4s produced the correct flash-then-solid-amber setup-mode result. The disable-networking state self-recovered (device rebooted and rejoined its home network unattended) without needing a manual re-enable step. | — |
| 2026-07-17 | **Finding: setup hotspot SSID is model-specific, not a fixed name** | Real-device capture found the Wireless Link Adapter broadcasts `Bose ST WLA (FFAD16)` — `WLA` abbreviating the product name, `FFAD16` the last 3 bytes of its MAC (`F4E11EFFAD16`). This is a different naming scheme than the generic "Bose SoundTouch Wi-Fi Network" documented for full speakers; the PWA's setup instructions should tell users to look for a `Bose ST <model> (<suffix>)`-shaped SSID, not one exact string. | — |
| 2026-07-17 | **Spike A real-device capture: setup UI behavior (Wireless Link Adapter, `http://192.0.2.1`)** | Confirmed via phone browser walkthrough: (1) the UI performs a **live WiFi scan** and shows a dropdown of all nearby SSIDs (not a single guessed network) with a "..." menu for further options (not explored — likely hidden-network manual entry); (2) the submit button renders with **no visible label** (blank white pill) — an accessibility gap worth avoiding in the PWA's own UI; (3) a wrong password shows an X + retry icon and **does not drop the hotspot**, allowing retry; a correct password shows a checkmark and only then ends the setup hotspot; (4) **the client device does not auto-reconnect to the home network after a successful join** — the phone had to be manually switched back to home WiFi; (5) the **adapter itself does** rejoin its home network and previous DHCP lease automatically and immediately (confirmed reachable at the same IP right after). The exact HTTP request/response of the join submission itself was not captured — mobile Safari has no devtools network tab; only the rendered UI and its outcome states were observed. A follow-up capture from a laptop with real devtools would be needed if Phase 1 requires the literal wire format rather than just replicating the observed UX (scan → pick network → enter password → submit → poll for success/failure → prompt user to manually reconnect). | Capturing via laptop directly (would have required disconnecting this session's own network — deferred in favor of a phone-based walkthrough, since only the phone needed to join the hotspot) |
| 2026-07-17 | **Spike A confirmed:** standard SoundTouch API (port 8090) reachable at `192.0.2.1:8090/info` during setup mode | Direct fetch from a phone browser returned the same `<info>` XML shape as normal operation (name, type "SoundTouch Wireless Link Adapter", serials, `margeAccountUUID`, `networkInfo`) — this was previously only documented in Bose support articles, now empirically confirmed against the real device. | — |
| 2026-07-17 | **Spike A fully resolved — exact WiFi-provisioning wire format recovered from the setup UI's own JS source** (`js/ap.js`, served by the device itself at `http://192.0.2.1/js/ap.js`) | A laptop connected directly to the setup hotspot (accepting the temporary loss of home network/internet) fetched the setup page's raw HTML and referenced JS/CSS assets via a small read-only capture script. `ap.js` contains the complete client logic: **network scan** — `POST http://<host>:8090/performWirelessSiteSurvey` with body `<PerformWirelessSiteSurvey timeout="5"/>` (`contentType: text/xml`); response is XML with one `<item ssid="…" signalStrength="…"><securityTypes><type>…</type></securityTypes></item>` per network (client-side de-dupes consecutive identical `ssid`+security entries and filters out any SSID starting with `"Bose "`). **Join submission** — `POST http://<host>:8090/addWirelessProfile` with body `<AddWirelessProfile><profile ssid="$SSID" password="$PASSWORD" securityType="$SECURITY"></profile></AddWirelessProfile>`; `password`/`securityType` attributes are omitted entirely when the network has no security. Confirmed valid `securityType` values from the security `<select>`: `none`, `wep`, `wpa_or_wpa2`. The UI's success/error panes (checkmark vs. X) are driven directly by this single AJAX call's jQuery success/error callback — no separate polling endpoint is used by this page for that purpose. Also found (but not referenced by `ap.js`, role unconfirmed): `GET http://<host>:8090/setup` returns `<setupStateResponse state="SETUP_AP_OOB" systemstate="SETUP_LANG_SET" />` — a real, previously-undocumented out-of-box/setup-state endpoint, possibly used by a different step of the flow (e.g. language selection) not exercised in this capture. This closes Spike A's last open item — Phase 1 can implement the join flow directly against `performWirelessSiteSurvey`/`addWirelessProfile` without needing to replicate the UI's exact UX. | Guessing at endpoint names (the earlier speculative-probe approach) — reading the device's own served JS was far more reliable and gave the exact wire format, not just a plausible guess |
...
| SoundTouch Wireless Link Adapter | **Single rear `Control` button.** See detailed subsection below — this is the project's real test device | See below | **Real-device tested (2026-07-17)** — corrects the manual's stated duration, see below |
...
#### SoundTouch Wireless Link Adapter — deep dive (2026-07-10 manual research, corrected 2026-07-17 by real-device test)

**Real-device correction (2026-07-17):** the manual's stated 8–10s hold for
"enter setup mode" did not match this unit's actual behavior. A 10–12s hold
landed on the disable-networking state instead (LED flashed once then went
fully off — the device rebooted and silently rejoined its home network on its
own after, no manual recovery needed). A retry holding for **~2–4s** produced
the correct result: LED flashed once then went solid amber (setup mode). Use
~2–4s as the practical target for this unit; treat the manual's 8–10s figure
as unreliable for the "enter setup mode" case specifically. The
"disable-networking" duration was not independently re-tested short of the
accidental repro above.
