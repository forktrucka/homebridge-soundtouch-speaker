...
| 2026-07-18 | Post-merge QA audit found `SoundTouchZoneAccessory._pruneOrphanService` (added silently during implementation of this plan — never appeared as its own checklist item above) had zero test coverage: no `src/zones/__tests__/SoundTouchZoneAccessory.test.ts` case exercised an `accessoryType` change across a restart, and the integration suites only ever construct zones with a fixed `accessoryType`. Added coverage (fresh switch, fresh lightbulb, switch→lightbulb, lightbulb→switch, and unchanged-type-across-restart no-churn cases) directly against `SoundTouchZoneAccessory.create()` using a stateful fake `PlatformAccessory` (`getService`/`addService`/`removeService` backed by a `Map`) that persists across two `create()` calls to simulate a restart. All cases passed against the existing implementation with no behavior changes — `_pruneOrphanService` correctly removes only the stale same-zone-different-type service and leaves everything else untouched. Plan-hygiene gap: this method should have had its own checklist line item (e.g. "Prune stale service on `accessoryType` change across restart, with test coverage") back on 2026-06-21 or whenever it was introduced — undocumented scope added during implementation is exactly the kind of thing the checklist exists to catch. | QA audit of already-merged PR #162 flagged the gap; test-only follow-up, no source change. | Silently patching a behavior fix during a "just add tests" pass (not applicable here — no bug found) |
