...
| 2026-07-18 | **`_applyDefaultSourceIfIdle()` was dormant on every real device** because `presetFromElement()` (`src/devices/SoundTouch/api/preset.ts`) checked/read the attribute `updateOn`, but the real Bose `/presets` XML uses `updatedOn` (confirmed via direct curl against a real Kitchen speaker: `<preset id="1" createdOn="1784432310" updatedOn="1784432310">…`). `presetFromElement()` therefore always returned `undefined`, so `API.getPresets()` always returned `[]` — confirmed directly against a device with 5 real presets. Root cause predates this feature (introduced backwards in PR #52, "fix: correct SoundTouch API spec mismatches"); it stayed dormant because `getPresets()` had zero production callers until this feature's `_applyDefaultSourceIfIdle()`. Fixed on `fix/preset-updated-on-attribute` (three `updateOn`→`updatedOn` corrections in `preset.ts`, plus the happy-path test fixtures in `preset.test.ts` and `api.test.ts` that had locked in the same typo, plus new regression tests parsing the real-device XML shape at both the `presetFromElement()` and `getPresets()` layers). | Real-device QA pass; see the fix PR for the exact diff and regression tests. | N/A — this is a bug-fix row, not a design decision. |
