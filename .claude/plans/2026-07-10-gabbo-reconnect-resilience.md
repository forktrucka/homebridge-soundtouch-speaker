...
| 2026-07-18 | Follow-up coverage added (QA audit of PR #146 found the merged fix's motivating scenario — bare `error` with no `close` — had zero test coverage): two unit tests added to `GabboClient.test.ts` that dispatch a synthetic `error` event directly on the client's underlying socket. One confirms an `error`-only disconnect (no `close`) still schedules a reconnect (`setTimeout(…, 5000)`); the other dispatches both `error` and `close` on the same connection and confirms `disconnected` fires exactly once and only one reconnect is scheduled, matching the `handledDisconnect` guard's documented behaviour. Both tests passed against the existing implementation on first run — no behavior change needed; the merged fix works as documented, this closes a coverage gap only | — |
