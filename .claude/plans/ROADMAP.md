...
| ----- | ---- | ------ | ------ | ----------------- |
| 1c | **Zone default source** | `feat/zone-default-source` | 🟢 Unblocked | Follow-up to Speaker zones (merged #162): optional per-zone `defaultSource` (a primary preset slot) played when a zone is activated and the primary is idle — fill-if-empty, never override. `feat:` → minor. Composes with the #162 zone power-on fix and #161 resume-last-source via the fill-if-empty rule. Soft affinity to [03] source selection (both build on `selectSource(contentItem)` + `getPresets()`), but no hard dependency. Must serialize (not run in parallel) with 1e Zone state reconciliation — both touch `SoundTouchZoneOnCharacteristic.ts`/`SoundTouchZoneAccessory.ts`. Plan: `plans/2026-07-18-zone-default-source.md`. |
...
| 1e | **Zone state reconciliation** | `fix/zone-state-reconciliation` | 🟢 Unblocked | Follow-up to Speaker zones (merged #162): `_isZoneActive()` gains a primary-power gate (`SoundTouchDevice.deviceIsOn(primary)` short-circuits to `false` before checking `getZone()` membership — if the primary is in standby, the zone reads off regardless of stale membership); zone accessories gain a dedicated always-on reconciliation poll (`ZONE_RECONCILIATION_INTERVAL_MS`, proposed 60s — shorter than the speaker's 5-min default since zones have no gabbo push wiring at all, so polling is the *only* correction path, not a backstop). `fix:` → patch. Must serialize (not run in parallel) with 1c Zone default source — both touch `SoundTouchZoneOnCharacteristic.ts`/`SoundTouchZoneAccessory.ts`. Plan: `plans/2026-07-18-zone-state-reconciliation.md`. |
